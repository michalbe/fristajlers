...
$conazwa = substr($conazwa, 0, 25);
	$query = "UPDATE F_sporty SET opis='$co', nazwa='$conazwa' WHERE id_wlasc='$id'"; 
        mysql_query($query); 
	$newsy = $newsy .'<center><br><br><font>Twoj opis zostal zaakceptowany.</font>';

} else {

		$query1 = "SELECT opis, nazwa FROM F_sporty WHERE id_wlasc='$id'"; 
        $wynik1 = mysql_query($query1); 
		$rekord1 = mysql_fetch_array ($wynik1);

$newsy = $newsy .'<form method="POST" action="main.php?lokacja=ks_opis">
